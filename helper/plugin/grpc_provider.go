...
func (s *GRPCProviderServer) ApplyResourceChange(_ context.Context, req *proto.ApplyResourceChange_Request) (*proto.ApplyResourceChange_Response, error) {
	...
	if plannedStateVal.IsNull() {
		...
		diff, err = schema.DiffFromValues(
			...
			func(is *terraform.InstanceState, c *terraform.ResourceConfig) {
				FixupAsSingleInstanceStateIn(is, s.provider.ResourcesMap[req.TypeName])
				fixupAsSingleResourceConfigIn(c, s.provider.ResourcesMap[req.TypeName].Schema)
			},
		)
		if err != nil {
			resp.Diagnostics = convert.AppendProtoDiag(resp.Diagnostics, err)
	...
	// fixupAsSingleInstanceDiffIn to the diff, so we need not (and must not)
	// repeat that here.
