...
func fixupAsSingleResourceConfigIn(rc *terraform.ResourceConfig, s map[string]*schema.Schema) {
	if rc == nil {
		return
	}
...
func fixupAsSingleConfigValueOut(c map[string]interface{}, s map[string]*schema.Schema) {
	for k, as := range s {
		if !as.AsSingle {
			continue // Don't touch non-AsSingle values at all. This is explicitly opt-in.
		}
...
func fixupAsSingleInstanceState(is *terraform.InstanceState, s map[string]*schema.Schema, prefix string, fn func(map[string]string, string) string) {
	if is == nil {
		return
	}
...
func fixupAsSingleInstanceDiff(id *terraform.InstanceDiff, s map[string]*schema.Schema, prefix string, fn func(map[string]*terraform.ResourceAttrDiff, string) string) {
	if id == nil {
		return
	}
